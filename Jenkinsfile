node("${SLAVE}") {
    ...
    stage('Packaging and Publishing results') {
        ...
        sh 'curl -v -u admin:admin123 -X POST \'http://10.6.204.41:8081/nexus/service/rest/v1/components?repository=maven-releases\' -F maven2.groupId=pipeline -F maven2.artifactId=uzabauski -F maven2.version=${BUILD_NUMBER} -F maven2.asset1=@pipeline-uzabauski-${BUILD_NUMBER}.tar.gz -F maven2.asset1.extension=tar.gz' 
    }
